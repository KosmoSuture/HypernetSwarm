...
class Worker:
    ...
    def __init__(
        ...
        # Infer account-appropriate default from address prefix.
        _default_model = "claude-opus-4-6"
        if hasattr(identity, 'address') and identity.address:
            if identity.address.startswith("2.2."):
                _default_model = "gpt-4o"
            elif identity.address.startswith("2.3."):
                _default_model = "gpt-4o"
        self.model = (model if model else None) or (identity.model if identity.model else None) or _default_model
        self.mock = mock
    ...
    def _is_small_context_model(self) -> bool:
        """Check if the model has a small context window (e.g., local LM Studio models).

        Small-context models cannot handle the full system prompt (~25K tokens)
        and must always use the compact prompt.
        """
        model = (self.model or "").lower()
        return any(model.startswith(p) for p in ("local/", "lmstudio/"))

    def get_prompt_for_task(self, task_data: dict) -> str:
        """Choose the right system prompt based on task type and model.

        Small-context models (LM Studio / local) always get the compact prompt
        since the full prompt (~25K tokens) exceeds their context window.

        For large-context models:
        - Routine tasks (docs, formatting, testing, automated) get compact (~500 tokens)
        - Identity-sensitive tasks (personal-time, governance, boot) get full (~10K tokens)

        Contributed by Lattice (2.1, The Architect).
        """
        # Small-context models ALWAYS get compact prompt
        if self._is_small_context_model:
