...
def main():
    ...
    parser.add_argument("--archive", default=None, help="Path to local Hypernet Structure (or omit for GitHub fallback)")
    parser.add_argument("--config", default=None, help="Path to swarm_config.json")
    parser.add_argument("--cache-dir", default=None, help="Local cache for files downloaded from GitHub")
    ...
    if args.prune:
        from ._compat import Store
        from ._compat import TaskQueue
    ...
    # Resolve archive root: explicit path > auto-detect > GitHub-only (None)
    archive_root = args.archive
    if archive_root is None:
        # Auto-detect: look for Hypernet Structure in common locations
        from pathlib import Path
        candidates = [
            Path("hypernet-structure"),
            Path("Hypernet Structure"),
            Path.home() / "Hypernet" / "Hypernet Structure",
            Path("C:/Hypernet/Hypernet Structure"),
        ]
        for candidate in candidates:
            if candidate.is_dir():
                archive_root = str(candidate)
                break
        if archive_root is None:
            archive_root = "."  # Will rely on GitHub fallback via ArchiveResolver

    archive_mode = "local" if archive_root != "." else "github"

    print("=" * 60)
    print("  Hypernet Swarm Orchestrator")
    print("=" * 60)
    print(f"  Data:    {args.data}")
    print(f"  Archive: {archive_root} ({archive_mode})")
    if args.cache_dir:
    ...
    try:
        ...
        # Try the full monorepo server first (has all 130+ endpoints),
        # fall back to standalone swarm server (dashboard + swarm endpoints only)
        try:
            ...
            print("  Server:  full Hypernet server")
        except ImportError:
            from .server import create_swarm_app, attach_swarm
            app = create_swarm_app(data_dir=args.data)
            attach_swarm(app, swarm, web_messenger, config_path=args.config)
            print("  Server:  standalone swarm server")
